...
function createNewCard(el) {
    ...
    },
    {
        ...
        handleLikeClick: () => {
            const resApi = card.isCardLikedThisUser() ? api.deleteLike(card.getIdCard()) : api.likeCard(card.getIdCard());
            resApi.then(data => {
...
const confirmPopup = new PopupConfirm('.popup_delete',
    (card) => {
...
const profileFormSubmit = () => {
    ...
    api.setUserInfo(userInfo.name, userInfo.occupation)
        .then((res) => {
...
const profilePopupWithForm = new PopupWithForm('.popup_profile', profileFormSubmit);
...
const avatarFormSubmit = () => {
    ...
    api.setAvatar(inputAvatarLink.value)
        ...
        .catch(err => console.log(err))
        .finally(() => avatarPopupWithForm.setDefaultBtnText());
}

//попап редактирования аватара
const avatarPopupWithForm = new PopupWithForm('.popup_avatar', avatarFormSubmit);

//обработчик формы создания карточки
const createFormSubmit = () => {
    createPopupWithForm.setBtnText('Создание...');
    api.setNewCard(addCardName, addCardLink)
        .then((res) => {
        ...
        .catch(err => console.log(err))
        .finally(() => createPopupWithForm.setDefaultBtnText());
}
